...
test.describe("Tornado synthetic demo recording · read only", () => {
  ...
  test("UAT-03 Casino Operations", async ({ page }) => {
    ...
    await test.step("Show assigned casinos and operational work", async () => {
  ...
  test("UAT-04 Workforce", async ({ page }) => {
    await signIn(page, accounts.supervisor);
    await test.step("Show recorded staffing coverage without changing assignments", async () => {
      await visit(page, "/operations", "Operations command");
      await expect(page.getByRole("heading", { name: "Night shift staffing" })).toBeVisible();
      await capture(page, "04-workforce-coverage");
    });
  });

  test("UAT-05 WhatsApp and cleaning evidence", async ({ page }) => {
    await signIn(page, accounts.supervisor);
    await test.step("Show the seeded evidence review and source labels", async () => {
  ...
  test("UAT-06 Equipment", async ({ page }) => {
    await signIn(page, accounts.supervisor);
    await test.step("Show report-only equipment intake", async () => {
  ...
  test("UAT-08 Finance and RBAC", async ({ browser }) => {
    ...
    try {
      ...
      await test.step("Finance-showcase Supervisor is denied finance", async () => {
        await signIn(supervisor, accounts.financeSupervisor);
  ...
  test("UAT-10 Existing AI functionality", async ({ page }) => {
    ...
    await test.step("Show labelled Mock AI decision support", async () => {
      ...
      if (await page.getByRole("heading", { name: "Prepare the synthetic evidence pair" }).isVisible()) {
        ...
        test.skip(true, "Mock AI controls appear only after a synthetic evidence pair exists; this recorder does not mutate the shared demo.");
      }
