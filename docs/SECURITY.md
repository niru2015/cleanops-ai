...
CLEAN-034 stores original contracts in a private bucket. Only a Director or assigned Area Manager can stage, finalize, extract or obtain a short signed download URL for an authorized version. Server-side finalization verifies the uploaded bytes; the browser cannot write verified metadata or extraction/decision rows directly. Operations Managers can view and review operational proposal snippets without raw documents or commercial values. The review RPC checks role and draft status before canonical changes. The extractor receives source text only and has no credentials or action tools; document instructions are inert data. Local scenario files are synthetic and make no claim about real customer agreements.

CLEAN-035 keeps expense receipts in a private bucket with a 10 MiB JPEG/PNG/WebP/PDF allowlist. A scoped upload ticket and server byte verification precede a ready document. Signed downloads first check the requesting user's finance access to the linked intake. Candidates, claims, allocation and posting rows are read-only to browser roles; security-definer RPCs check active membership and site grants for every consequential transition. Area Managers can resolve their assigned sites but cannot approve; Directors can approve organization-wide. The transaction rejects missing receipts, unbalanced allocations and a receipt hash already posted in the organization. Posting and audit rows are immutable. Source messages and extracted fields remain untrusted and private; public or client-facing reports do not expose them.
