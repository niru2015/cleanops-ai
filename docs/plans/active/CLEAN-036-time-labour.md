...
Checks: clean local database reset; pgTAP 370/370; scenario generate/assert/reset; 4 authenticated Chromium checks for Director, Area Manager and Supervisor; `typecheck`, `lint`, 76 unit tests, `build`, and local `db lint --fail-on error` passed. The local lint reported two pre-existing warnings in `fail_processing_job` and `fail_whatsapp_reply`. Browser checks used local synthetic logins; the hosted app was only observed signed out. A later E2E run changes local seed memberships, so reset before repeating pgTAP.
