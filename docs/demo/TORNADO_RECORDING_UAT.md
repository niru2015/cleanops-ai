...
## Hosted recorder snapshot — alternate seed 20260927

Against `https://cleanops-ai.vercel.app` with version 8 `finance-showcase` run `cf17ee03-dfe7-59f6-8d18-faf51007d919`, seed `20260927`, the recorder produced 13 passed, one skipped and zero failed chapters. UAT-05B verified “Submission ready for review” using the existing two clearly labelled synthetic images on the separate Slot Bank 14 task. The current run's HTML, JSON, JUnit, 17 screenshots, native videos and traces are under `artifacts/tornado-demo/` in the operator workspace; they are ignored by Git. The eight-chapter finance overview video was also rerecorded after the alternate-seed replay: all eight routes returned HTTP 200 and the browser recorded zero console errors.
