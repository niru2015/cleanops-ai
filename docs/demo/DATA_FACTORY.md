...
**CLEAN-018 adapter blocker:** This equipment-history branch adds the persisted workflow and synthetic database lifecycle tests, but the #28 scenario parser still rejects `modules.equipment`. It does not yet generate one healthy asset and one repeat-repair Harbour asset with dated source invoices. Add that adapter and reset/assert coverage in a separate #28 change before claiming the requested equipment demo scenario or #34 repair exception is ready.
