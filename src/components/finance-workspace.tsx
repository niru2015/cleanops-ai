...
export function FinanceWorkspace({ workspace, editable, siteId }: { workspace: FinanceWorkspace; editable: boolean; siteId: string }) {
  ...
  return (
    <div className="financeWorkspace">
      <header className="financeHeader"><div><p className="eyebrow">{editable ? "Director-controlled records" : "Area Manager financial view"}</p><h1>Finance &amp; inventory</h1><p>{editable ? "Record supplier receipts, consumption and labour against the selected casino. Totals are derived in the database." : "Financial records for the selected casino are read only for Area Managers."}</p></div><span className="recordLabel">{workspace.items.length} active items</span></header>
      ...
      <div className="financeGrid">\n        {editable ? <>
        <section className="financePanel" aria-labelledby="inventory-entry-title">
          ...
          <form className="financeForm" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); act({ action: "record_inventory", siteId, vendorId: String(form.get("vendorId") || "") || undefined, inventoryItemId: String(form.get("inventoryItemId")), transactionType: String(form.get("transactionType")) as "receipt" | "issue" | "adjustment" | "count", quantity: Number(form.get("quantity")), unitCost: Number(form.get("unitCost")), occurredAt: now(), notes: String(form.get("notes") || "") || undefined }); event.currentTarget.reset(); }}>
