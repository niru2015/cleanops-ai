...
export function MessageContextQueue({ workspace }: { workspace: MessageWorkspace }) {
  ...
  return (
    <section className="messageQueue" aria-labelledby="message-queue-title">
      <header className="messageQueueHeader"><div><p className="eyebrow">WhatsApp normalization queue</p><h2 id="message-queue-title">Message context review</h2><p>Casino site is suggested from the registered receiving number. A supervisor confirms the area, task and sender before the message is used for operations or finance.</p></div><span className="recordLabel">{workspace.messages.length} visible</span></header>
