...
export async function acceptFinanceImport(input: unknown): Promise<FinanceActionState> {
  ...
  try {
    ...
    const staged = await client.rpc("stage_finance_csv_import", {
      p_organization_id: access.organizationId, p_source_file_name: parsed.data.fileName, p_source_file_hash: hash,
...
export async function performFinanceAction(input: FinanceActionInput): Promise<FinanceActionState> {
  ...
  try {
    ...
    if (parsed.data.action === "create_vendor") {
      const { error } = await client.from("vendors").insert({ organization_id: access.organizationId, name: parsed.data.name, vendor_code: parsed.data.vendorCode ?? null, contact_reference: parsed.data.contactReference ?? null });
      if (error) throw error;
      revalidatePath("/finance");
      return success("Vendor added to the organization catalogue.");
    }
    if (parsed.data.action === "create_inventory_item") {
      const { error } = await client.from("inventory_items").insert({ organization_id: access.organizationId, sku: parsed.data.sku ?? null, name: parsed.data.name, category: parsed.data.category ?? null, unit_of_measure: parsed.data.unitOfMeasure, reorder_level: parsed.data.reorderLevel });
      if (error) throw error;
      revalidatePath("/finance");
      return success("Inventory item added to the organization catalogue.");
    }
    if (parsed.data.action === "record_inventory") {
      const { error } = await client.from("inventory_transactions").insert({ organization_id: access.organizationId, site_id: parsed.data.siteId, vendor_id: parsed.data.vendorId ?? null, inventory_item_id: parsed.data.inventoryItemId, transaction_type: parsed.data.transactionType, quantity: parsed.data.quantity, unit_cost: parsed.data.unitCost, occurred_at: parsed.data.occurredAt, notes: parsed.data.notes ?? null });
      if (error) throw error;
      revalidatePath("/finance");
    ...
    if (parsed.data.action === "record_labour") {
      const { error } = await client.from("labor_cost_entries").insert({ organization_id: access.organizationId, site_id: parsed.data.siteId, worker_id: parsed.data.workerId ?? null, task_run_id: parsed.data.taskRunId ?? null, work_date: parsed.data.workDate, hours: parsed.data.hours, hourly_cost: parsed.data.hourlyCost, cost_type: parsed.data.costType, notes: parsed.data.notes ?? null });
      if (error) throw error;
      revalidatePath("/finance");
      return success("Labour cost recorded. Hours and cost rate determine the saved total.");
    }
    if (parsed.data.action === "update_inventory") {
      const { error, count } = await client.from("inventory_transactions").update({ vendor_id: parsed.data.vendorId ?? null, inventory_item_id: parsed.data.inventoryItemId, transaction_type: parsed.data.transactionType, quantity: parsed.data.quantity, unit_cost: parsed.data.unitCost, occurred_at: parsed.data.occurredAt, notes: parsed.data.notes ?? null }, { count: "exact" }).eq("id", parsed.data.id).eq("organization_id", access.organizationId).eq("site_id", parsed.data.siteId);
      if (error) throw error;
    ...
    if (parsed.data.action === "delete_inventory") {
      const { error, count } = await client.from("inventory_transactions").delete({ count: "exact" }).eq("id", parsed.data.id).eq("organization_id", access.organizationId).eq("site_id", parsed.data.siteId);
      if (error) throw error;
      if (!count) return failure("That inventory transaction could not be found for this casino.");
      revalidatePath("/finance");
    ...
    const { error, count } = await client.from("labor_cost_entries").delete({ count: "exact" }).eq("id", parsed.data.id).eq("organization_id", access.organizationId).eq("site_id", parsed.data.siteId);
    if (error) throw error;
    if (!count) return failure("That labour cost entry could not be found for this casino.");
    revalidatePath("/finance");
...
export async function performMessageResolution(input: MessageResolutionInput): Promise<FinanceActionState> {
  ...
  try {
    ...
    await resolveMessageContext(client, getFinanceSiteContext(access, parsed.data.siteId), parsed.data);
    revalidatePath("/finance");
